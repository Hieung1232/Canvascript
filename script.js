  ...
  if (space) {
    ...
    while (i < canvasArray.length) {
      ...
      if (int(CA1) && int(CA2)) {
        ...
        space.innerHTML += `<div>ctx.lineTo(${canvasArray[i + 2]}, ${
          canvasArray[i + 3]
        });</div>`;
        i += 4;
      } else if (Array.isArray(CA1)) {
...
function showSavePopup() {
  const popup = document.getElementById("saveNotification");
